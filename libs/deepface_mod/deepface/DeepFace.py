...
def frame_leve_processing(img,detector='SSD', enable_face_analysis = True, source = 0, time_threshold = 5, frame_threshold = 5):
	...
	#realtime.analysis(db_path, model_name, distance_metric, enable_face_analysis, source = source, time_threshold = time_threshold, frame_threshold = frame_threshold)
	prob_emotion = realtime.facial_analysis(enable_face_analysis, source = source, time_threshold = time_threshold, frame_threshold = frame_threshold)

def stream(detector='SSD', enable_face_analysis = True, source = 0, time_threshold = 5, frame_threshold = 5):
	...
	#realtime.analysis(db_path, model_name, distance_metric, enable_face_analysis, source = source, time_threshold = time_threshold, frame_threshold = frame_threshold)
	realtime.facial_analysis(enable_face_analysis, source = source, time_threshold = time_threshold, frame_threshold = frame_threshold)
...
def detectFace(img_path, detector_backend = 'mtcnn', enforce_detection = True):
	...
	img,region = functions.preprocess_face(img = img_path, detector_backend = detector_backend
		, enforce_detection = enforce_detection)#preprocess_face returns (1, 224, 224, 3)
	return img[:, :, ::-1], region#bgr to rgb
